...
def generate_launch_description():
    ...
    declare_slam_params_file_cmd = DeclareLaunchArgument(
        'slam_params_file',
        default_value=os.path.join(get_package_share_directory("rover_gazebo"),
    ...
    # slam_toolbox wants a 2D LaserScan; the rover carries a 16-ring 3D lidar, so
    # flatten a horizontal band of it into /scan (see config/pointcloud_to_laserscan.yaml).
    pointcloud_to_laserscan_node = Node(
        package='pointcloud_to_laserscan',
        executable='pointcloud_to_laserscan_node',
        name='pointcloud_to_laserscan',
        output='screen',
        parameters=[os.path.join(get_package_share_directory("rover_gazebo"),
