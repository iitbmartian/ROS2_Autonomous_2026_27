...
Ten of the eleven are **placeholder directories** — each holds only a README stating its scope
...
The placeholder packages have no `package.xml` and are skipped. Of the vendored drivers,
`sbg_driver` and `zed_msgs` build once `rosdep` has run.
