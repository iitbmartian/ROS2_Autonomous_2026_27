...
here, in one place, with a comment saying why:
  ...
  * loop-closing joints become <mimic> constraints
...
DECOR_MIMIC = ["LCON", "RCON", "Differential"]
...
FOLLOWER_LIMIT = 0.60             # rad, generous so a mimic never fights its own stop
STEER_LIMIT = 1.75                # rad, just past a quarter turn for spot mode
EFFORT = 500.0                    # N m, steering and drive motors
# The rockers are not motors. Under coupling:=controller their "actuator" stands in
# for a steel connector bar and a differential, so the limit has to be structural
# rather than motor sized, or the coupling saturates and the suspension sags.
...
class Gen:
    ...
    def emit_joint(self, el):
        ...
        if child in DECOR_MIMIC:
            leader = "FLS_joint"    # never chain a mimic onto another mimic
            self.w("  <!-- decorative; follows the free rocker so the linkage looks right -->")
            self.w('  <xacro:if value="${coupling == \'mimic\'}">')
            self.w(f'    <joint name="{name}" type="revolute">')
            self.w(f'      <origin xyz="{vec(o.get("xyz"))}" rpy="{vec(o.get("rpy"), 12)}"/>')
            self.w(f'      <parent link="{parent}"/>')
            self.w(f'      <child link="{child}"/>')
            self.w(f'      <axis xyz="{vec(axis)}"/>')
            self.w(f'      <limit lower="-1.0" upper="1.0" effort="{EFFORT}" velocity="{VELOCITY}"/>')
            self.w("      <dynamics damping=\"0.5\" friction=\"0.0\"/>")
            self.w(f'      <mimic joint="{leader}" multiplier="{fmt(mim[child], 6)}" offset="0"/>')
            self.w("    </joint>")
            self.w("  </xacro:if>")
            self.w('  <xacro:unless value="${coupling == \'mimic\'}">')
        ...
        if raw in ROCKERS:
            self.w(f"  <!-- follower: {raw} = {mim[raw]:+.5f} x FLS -->")
            head("revolute")
            limits(FOLLOWER_LIMIT, ROCKER_EFFORT)
            self.w('      <dynamics damping="5.0" friction="0.5"/>')
            self.w('      <xacro:if value="${coupling == \'mimic\'}">')
            self.w(f'        <mimic joint="FLS_joint" multiplier="{fmt(mim[raw], 6)}" offset="0"/>')
            self.w("      </xacro:if>")
            self.w("  </joint>")
            self.w()
            return
