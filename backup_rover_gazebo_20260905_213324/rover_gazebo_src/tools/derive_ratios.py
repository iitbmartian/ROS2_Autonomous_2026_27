...
joint couplings, which a URDF *can* carry as <mimic> tags.
...
def derive(path):
    ...
    return {
        ...
        "mimic": {
            ...
            # Expressed against FLS, not against its own parent rocker. A mimic whose
            # leader is itself a mimic may not resolve, so every follower in the model
            # points at the one free joint.
...
def emit_yaml(d, out):
    ...
    lines += [
        "",
        "  # Suspension coupling. FLS is the single free coordinate; the rest are mimics.",
        ...
        f"      {j('BRS')}: {f(m['BRS'])}",
        f"      {j('FRS')}: {f(m['FRS'])}",
        "    # Raw pairwise relations, before they are chained onto the leader. The",
        "    # fallback coupling node needs these, because it applies each constraint",
...
def report(d):
    ...
    out.append(f"  {D['law']}")
    out.append("")
    out.append("Mimic multipliers, all against FLS")
    for k in ("BLS", "BRS", "FRS"):
        out.append(f"  {k:<13s} {d['mimic'][k]:+.5f}")
    out.append("  decorative")
    for k in ("LCON", "RCON", "Differential"):
        out.append(f"    {k:<11s} {d['mimic'][k]:+.5f}")
    out.append("  every follower points at FLS directly; none is a mimic of a mimic")
    out.append("")
