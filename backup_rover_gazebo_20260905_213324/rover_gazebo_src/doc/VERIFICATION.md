...
## What could not be tested, and why it matters

The target is Gazebo Harmonic. Harmonic was not available on the machine this package
was built on, which had Gazebo Fortress and Gazebo Classic 11. Fortress's SDF version
has no `<mimic>` element at all and its physics plugin has no mimic constraint, so
**the default `coupling:=mimic` path has never been run.**

Everything about that path that could be checked without Harmonic, was:

- the multipliers are derived from the CAD by `tools/derive_ratios.py`, which
  independently confirms the linkage closes: it computes both differential pushrod
  lengths from the joint origins and gets 0.150000 m against a modelled 0.15 m
- the `<mimic>` tags are present and well formed in the generated URDF, six of them,
  every one pointing at the free joint directly rather than at another follower
- `robot_state_publisher` resolves them. Driving the free suspension joint to ten
  degrees moves all four wheels and the differential bar, in the pattern the analysis
  predicts

What is untested is the last link in the chain: whether Harmonic's URDF converter
carries those tags through into the solver. Older converters drop them silently.
`tools/check_setup.sh` answers that on your machine in under a minute, and
`tools/make_sdf.py` writes a model with them put in directly if the converter will not.

Everything below was run on **Gazebo Fortress with the `coupling:=controller`
fallback**, ROS 2 Humble, 1 ms physics step, DART. Each row is a separate simulator
launched from scratch, because runs left to share a simulator contaminate each other.
Distances are in the rover's own start frame: forward, then sideways.
...
## Where the fallback falls down

The fallback closes a stiff constraint loop over ROS topics at 500 Hz instead of inside
the physics step. Two consequences, both measured.
...
None of this applies to the mimic path. A solver constraint has no residual to push
with, so it has nothing to leak.
...
```bash
ros2 launch rover_gazebo rover_sim.launch.py coupling:=controller gui:=false &
