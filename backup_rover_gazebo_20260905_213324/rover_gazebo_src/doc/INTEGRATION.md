...
| `/imu` | `sensor_msgs/Imu` | out | simulated IMU on the chassis, bridged |
...
be integrated from `/joint_states`, which carries all eight driven joints plus the free
suspension joint.
...
Sensors go in `urdf/rover_gazebo.xacro` next to the IMU, as a `<gazebo reference=...>`
...
**The rocker joints are not actuators.** Under `coupling:=mimic` they are passive and
have no command interface at all. Do not spawn a controller that claims them.
