...
Two paths, chosen with the `coupling` launch argument.

**`coupling:=mimic`, the default.** The multipliers become `<mimic>` tags. Gazebo
Harmonic can enforce these in the constraint solver, which means they are exact, cost
nothing to hold, and transmit load correctly in both directions. This is the better
answer and it is the one to use if it works.

**`coupling:=controller`, the fallback.** The four rockers get effort interfaces and
`rocker_coupling_node` applies each constraint as a stiff spring and damper, pushing
equally and oppositely on the two joints it ties together:
...
This path works on any Gazebo version, but it is a compromise: it closes a stiff loop
over ROS topics at 500 Hz rather than inside the physics step, so the constraint is held
to a degree or two rather than exactly, and the gains cannot be raised much before the
loop delay makes it ring. The mimic path has neither limitation.
...
visible differential is easier to reason about than one without, but:

- the connector bars and the differential bar follow the free rocker through their own
  mimic tags, so the linkage still moves correctly on screen
...
Under `coupling:=controller` the decorative joints become fixed instead, since there is
